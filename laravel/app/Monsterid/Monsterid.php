...
namespace App\Monsterid;

use Illuminate\Support\Facades\Storage;;

class Monsterid
{
    public static function build_monster($seed='',$size='', $fileName = ''){
        // init random seed
        ...
        if($size && $size < 400){
            ...
            imagepng($out, 'images/user/'.$fileName.'.png');
            imagedestroy($out);
            imagedestroy($monster);
            return 'images/user/'.$fileName.'.png';
        }else{

            header ("Content-type: image/png");
            imagepng($monster);
            imagepng($monster, 'images/user/'.$fileName.'.png');
            imagedestroy($monster);
            return 'images/user/'.$fileName.'.png';
        }
    }
